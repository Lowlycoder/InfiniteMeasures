<div class="grid-container">




    <!--début de la sidebar-->
    <aside id="sidebar">
        <div class="sidebar-title">
            <div class="sidebar-brand">
                <span class="material-icons-outlined">home</span> Bonjour, john jones
            </div>
            <span class="material-icons-outlined" onclick="closeSidebar()">close</span>
        </div>
        <ul class="sidebar-list">
            <li class="sidebar-list-item">
                <a href="/dashboard"><span class="material-icons-outlined">dashboard</span>Tableau de Bord</a>
            </li>
            <li class="sidebar-list-item">
                <a href="/capteursUser"><span class="material-icons-outlined">cable</span>Capteurs</a>
            </li>
            <li class="sidebar-list-item">
                <a href="/Réglages"><span class="material-icons-outlined">settings</span>Réglages</a>
            </li>
        </ul>
    </aside>
    <!--fin de la sidebar-->




    <!--début du milieu de page-->
    <main class="main-container">




    </main>
</div>